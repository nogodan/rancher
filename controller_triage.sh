...
banner "4.5 / DNS Resolution"

# Discover cluster DNS IP — never use host /etc/resolv.conf on a controller node
CLUSTER_DNS_IP=""
for f in /var/lib/rancher/rke2/agent/etc/kubelet.env \
          /var/lib/rancher/k3s/agent/etc/kubelet.env; do
  [[ -z "$CLUSTER_DNS_IP" && -f "$f" ]] && \
    CLUSTER_DNS_IP=$(grep -oP '(?<=--cluster-dns=)\S+' "$f" 2>/dev/null | head -1 || true)
done
...
if [[ -z "$CLUSTER_DNS_IP" ]] && check_cmd kubectl && \
    kubectl cluster-info &>/dev/null 2>&1; then
  CLUSTER_DNS_IP=$(kubectl get svc kube-dns -n kube-system \
    -o jsonpath='{.spec.clusterIP}' 2>/dev/null || true)
fi
[[ -z "$CLUSTER_DNS_IP" ]] && CLUSTER_DNS_IP="10.43.0.10"
info "DNS resolution check (cluster DNS: ${CLUSTER_DNS_IP})"

if check_cmd nc; then
  if nc -z -w 3 "${CLUSTER_DNS_IP}" 53 &>/dev/null 2>&1; then
...
if check_cmd nslookup; then
  if nslookup kubernetes.default.svc.cluster.local "${CLUSTER_DNS_IP}" &>/dev/null 2>&1; then
  ...
  if dig +short +timeout=3 @"${CLUSTER_DNS_IP}" kubernetes.default.svc.cluster.local &>/dev/null 2>&1; then
    ok "In-cluster DNS: kubernetes.default.svc.cluster.local resolves via ${CLUSTER_DNS_IP}"
  else
    warn "In-cluster DNS: query to ${CLUSTER_DNS_IP} failed — check CoreDNS pods"
  fi
...
if check_cmd systemctl; then
  # Check RKE2/k3s services first — determine which runtime manages containerd
  RKE2_OR_K3S_ACTIVE=false
  for svc in rke2-server rke2-agent k3s k3s-agent; do
  ...
  # kubelet — embedded in RKE2/k3s; standalone unit only on kubeadm setups
  if systemctl list-unit-files "kubelet.service" &>/dev/null \
      && systemctl list-unit-files "kubelet.service" | grep -q "kubelet"; then
    STATE=$(systemctl is-active kubelet 2>/dev/null || echo "unknown")
    case "$STATE" in
      active)   ok   "kubelet: active" ;;
      inactive)
        if [[ "$RKE2_OR_K3S_ACTIVE" == true ]]; then
          info "kubelet: inactive systemd unit — embedded in RKE2/k3s (expected)"
        else
          fail "kubelet: inactive — not managed by RKE2/k3s either"
        fi
        ;;
  ...
  # containerd is an internal component of rke2-server/rke2-agent/k3s —
  ...
  if [[ "$RKE2_OR_K3S_ACTIVE" == false ]] && systemctl cat docker.service &>/dev/null 2>&1; then
...
# ── Threshold Reference ───────────────────────────────────────
# Use helper functions so printf format strings never contain ANSI codes
# (ANSI codes inside printf format corrupt column width calculations)
trow3() { printf "  │ %-31s │ %-12s │ %-12s │\n" "$1" "$2" "$3"; }
trow2() { printf "  │ %-31s │ %-28s │\n" "$1" "$2"; }
trow2w() { printf "  │ %-43s │ %-13s │\n" "$1" "$2"; }
...
echo    "  ├─────────────────────────────────┼──────────────────────────────┤"
trow2   "fsync latency p99 (fio)"   "< 10ms  (fail > 10ms)"
trow2   "Write speed (dd fallback)" ">= 50 MB/s"
echo    "  └─────────────────────────────────┴──────────────────────────────┘"
echo ""
...
echo    "  ├─────────────────────────────────────────────┼───────────────┤"
trow2w  "net.ipv4.ip_forward"                   "1"
trow2w  "net.bridge.bridge-nf-call-iptables"    "1"
trow2w  "net.bridge.bridge-nf-call-ip6tables"   "1"
trow2w  "vm.swappiness"                         "0"
trow2w  "br_netfilter module"                   "loaded"
trow2w  "overlay module"                        "loaded"
echo    "  └─────────────────────────────────────────────┴───────────────┘"
